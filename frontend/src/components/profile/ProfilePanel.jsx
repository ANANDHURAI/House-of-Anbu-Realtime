...
function ProfilePanel({ user, onClose, onLogout, onProfileUpdated }) {
  ...
  return (
    <div className="fixed inset-0 bg-[#050505] z-50 flex flex-col animate-in fade-in slide-in-from-right duration-300">
      ...
      {toast.show && (
        <div className={`fixed top-10 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-2xl shadow-2xl z-[60] flex items-center gap-3 animate-in fade-in slide-in-from-top-5 duration-300 ${
          toast.type === "success" ? "bg-[#D4AF37] text-black" : "bg-red-500 text-white"
        }`}>
          {toast.type === "success" ? (
