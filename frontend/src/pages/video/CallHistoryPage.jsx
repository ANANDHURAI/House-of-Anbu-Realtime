    ...
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-4">
        {callHistory.map((call) => {
          // Determine who the current user is in this call
          const isOutgoing = call.caller === currentUser?.id;

          // Always show the OTHER person's info
          const otherPersonName = isOutgoing ? call.receiver_name : call.caller_name;
          const otherPersonImage = isOutgoing ? call.receiver_image : call.caller_image;

          // For call-back, always call the other person
          const callBackReceiverId = isOutgoing ? call.receiver : call.caller;

          // Sub-label under the name
          const subLabel = call.is_missed
            ? isOutgoing
              ? 'You started the call'   // caller whose call was rejected/missed
              : 'Missed Call'             // receiver who missed the incoming call
          ...
          return (
            <div key={call.id} className="bg-[#121212] border border-[#222] rounded-3xl p-5 hover:border-[#D4AF37]/50 transition-all flex items-center gap-5">
              <div className="w-14 h-14 rounded-full border-2 border-[#D4AF37]/30 p-0.5 shadow-lg">
                <img
                  src={otherPersonImage || "https://cdn-icons-png.flaticon.com/512/149/149071.png"}
                  className="w-full h-full rounded-full object-cover"
                  alt={otherPersonName}
                />
              </div>
              <div className="flex-1">
                <h3 className={`font-bold ${call.is_missed ? 'text-red-500' : 'text-white'}`}>
                  {otherPersonName}
                </h3>
                <p className="text-[10px] text-gray-500 uppercase tracking-widest font-black mt-1">
                  {subLabel} • {formatDate(call.started_at)}
                </p>
              </div>
              <button
                onClick={() => handleCallAgain(callBackReceiverId)}
                className="p-4 bg-[#D4AF37]/10 text-[#D4AF37] border border-[#D4AF37]/20 rounded-2xl hover:bg-[#D4AF37] hover:text-black transition-all"
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {callHistory.length === 0 && (
          <div className="text-center text-gray-600 py-20 text-sm uppercase tracking-widest font-bold">
            No call history yet
          </div>
        )}
      </div>
